use bevy::prelude::*;

// Window / world bounds
pub const WINDOW_WIDTH: u32 = 1280;
pub const WINDOW_HEIGHT: u32 = 720;
pub const BOUNDS_X: f32 = (WINDOW_WIDTH as f32) / 2.0;
pub const BOUNDS_Y: f32 = (WINDOW_HEIGHT as f32) / 2.0;

// Player
pub const PLAYER_DIMENSION: Vec2 = Vec2::new(60.0, 46.0);
pub const PLAYER_HEALTH: f32 = 100.0;
pub const PLAYER_SPEED: f32 = 620.0;
pub const PLAYER_Y_COORDINATE: f32 = -BOUNDS_Y + 60.0;
pub const PLAYER_PROJECTILES_PER_SECOND: f32 = 5.0;
pub const PLAYER_PROJECTILE_SPEED_PER_SECOND: f32 = 1000.0;

// Enemies
pub const ENEMY_DIMENSION: Vec2 = Vec2::new(55.0, 48.0);
pub const ENEMY_SPEED: f32 = 130.0;
pub const ENEMY_DAMAGE: f32 = 35.0;
pub const ENEMY_SPAWNS_PER_SECOND: f32 = 1.2;
pub const ENEMY_MAX_COUNT: usize = 8;
pub const ENEMY_PROJECTILES_PER_SECOND: f32 = 0.7;
pub const ENEMY_PROJECTILE_SPEED_PER_SECOND: f32 = 380.0;

// Scores
pub const SCORE_PER_ENEMY: u32 = 100;

// Collision radius
pub const PROJECTILE_RADIUS: f32 = 7.0;

// Background
pub const SMALL_STAR_BACKGROUND_SPEED: f32 = 30.0;